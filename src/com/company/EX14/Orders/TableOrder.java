...
public class TableOrder implements Order {
	...
	public static void main(String[] args) {
		...
		Order order = new TableOrder(new Customer(23));
		order.add(item1);
		order.add(item2);
		order.add(item3);
		order.add(item4);
		order.removeAll(item3);
		order.add(new Dish("Potato", "Backed"));
		order.add(new Dish("Porridge", "Grechka"));
		order.add(new Dish("Potato", "Backed"));
		order.removeAll(new Dish("Potato", "Backed"));
		Item[] name = order.getItems();
